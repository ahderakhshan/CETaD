...
from Evaluators.Evaluators import EvaluateBySiamese
...
class RepresentativePointSelector:
    def __init__(self, siamese_model, optimizer_iterations, optimizer_learning_rate, alpha, beta):
...
class ContradictionExtractor:
    def __init__(self, siamese_model, tokenizer_max_length, sentences, representative_point_selector: RepresentativePointSelector,
                 terms_contradiction_path, ppmi_contradiction_path,
        ...
        self.embeddings = self.siamese_model.embed_sentences(sentences, tokenizer_max_length)
        ...
        self.representative_point_selector.set_embeddings(self.embeddings)
    ...
    def evaluate_by_siamese(self):
        pairs = self.extract_contradictory_pairs()
        sum_e, sum_n, sum_c = EvaluateBySiamese(self.siamese_model, pairs, self.embeddings).evaluate()
        logging.info(f"entailment prob={sum_e}, neutral_prob={sum_n}, contradiction prob={sum_c}")
